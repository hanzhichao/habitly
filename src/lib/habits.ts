...
async function _getHabitsWithStatusToday() {
...
async function _getHabitsWithStatus(day: string) {
  ...
  const habits = await db.select<Habit[]>(`
    ...
    WHERE date('now', 'localtime') >= date(start_date, 'localtime');`, [day])
...
export async function getTodayHabits()
{
  ...
  const result = await _getHabitsWithStatusToday()
  const habits: Habit[] = [];
  const now = new Date();
...
export async function addHabit(name: string, goal: string, frequency: string, reminder: string, icon: string, color: string) {
  await info(`addHabit: name=${name}`)
  const db = await getDb();
  return await db.execute("INSERT INTO `habits` (name,goal,frequency,reminder,icon,color,start_date) VALUES ($1,$2,$3,$4,$5,$6,date('now', 'localtime'));", [name, goal, frequency, reminder, icon, color]);
}
...
export async function updateHabit(habitId: number, name: string, goal: string, frequency: string, reminder: string, icon: string, color: string) {
  await info(`updateHabit: habitId=${habitId}`)
  const db = await getDb();
...
export async function deleteHabit(habitId: number) {
  await info(`deleteHabit: habitId=${habitId}`)
  const db = await getDb();
  await db.execute("DELETE FROM `records` WHERE habit_id=$1", [habitId]);
  return await db.execute("DELETE FROM `habits` WHERE id=$1;", [habitId]);
}

// 查询习惯完成记录列表
export async function getCompletedRecords(habitId: number) {
  await info(`getCompletedRecords: habitId=${habitId}`)
  const db = await getDb();
  return db.select<Record[]>("SELECT * FROM `records` WHERE habit_id = $1 AND status = 1 ORDER BY date(record_date, 'localtime');", [habitId]);
}


// 查询习惯今日完成记录
async function _getRecordOfToday(habitId: number) {
  const db = await getDb();
  const records = await db.select<Record[]>("SELECT * FROM records WHERE habit_id=$1 AND records.record_date=date('now', 'localtime');", [habitId])
  if (records.length === 0) return
  return records[0]
}

// 添加习惯今日完成记录
async function _addRecordOfToday(habitId: number) {
  const db = await getDb();
  return db.execute("INSERT INTO records (habit_id,record_date) VALUES ($1,date('now', 'localtime'))", [habitId]);
}


// 更新习惯今日完成记录
async function _updateRecordOfToday(recordId: number, status: number) {
  const db = await getDb();
...
export async function toggleRecordStatusOfToday(habitId: number) {
  await info(`toggleRecordStatusOfToday: habitId=${habitId}`)
  const record = await _getRecordOfToday(habitId)
  if (typeof record !== "undefined") {
    const newStatus = record.status === 1 ? 0 : 1
    return await _updateRecordOfToday(record.record_id, newStatus)
  }
  return _addRecordOfToday(habitId)
}
...
export async function getWeeklyData() {
  ...
  for (let i = 0; i < 7; i++) {
    if (i < now.getDay()) {
      ...
      const habits = await _getHabitsWithStatus(dateStr)
